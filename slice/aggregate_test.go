...
func testMaxTypes[T gokit.RealNumber](t *testing.T) {
	res := Max[T]([]T{1, 2, 3})
	assert.Equal(t, T(3), res)
}

// testMinTypes 只是用来测试一下满足 Min 方法约束的所有类型
func testMinTypes[T gokit.RealNumber](t *testing.T) {
	res := Min[T]([]T{1, 2, 3})
	assert.Equal(t, T(1), res)
}

// testSumTypes 只是用来测试一下满足 Sum 方法约束的所有类型
func testSumTypes[T gokit.RealNumber](t *testing.T) {
	res := Sum[T]([]T{1, 2, 3})
